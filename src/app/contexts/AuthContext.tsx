...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async (email: string, password: string, _role: UserRole) => {
    ...
    const { data: perfil } = await supabase
      .from('perfiles')
      .select('nombre_completo, tipo_usuario, bloqueado')
      .eq('id', data.user.id)
      .single();

    if (perfil?.bloqueado) {
      await supabase.auth.signOut();
      throw new Error('Esta cuenta ha sido bloqueada por el administrador.');
    }
    ...
    const loggedUser: User = {
      ...
      role: (perfil?.tipo_usuario || 'niño') as UserRole,
    };
    setUser(loggedUser);
